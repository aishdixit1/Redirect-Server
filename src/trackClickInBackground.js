...
export async function trackClickInBackground({ broadcastId, contactId, buttonText }) {
  ...
  if (!broadcastId || !contactId || !UUID_REGEX.test(broadcastId) || !UUID_REGEX.test(contactId)) {
  ...
  try {
    ...
    const { rows } = await client.query(validationQuery, [broadcastId, contactId]);
    const { valid_broadcast, valid_contact } = rows[0];

    if (!valid_broadcast || !valid_contact) {
      console.warn(`Validation failed. Broadcast valid: ${valid_broadcast}, Contact valid: ${valid_contact}`);
    ...
    const decodedButtonText = buttonText ? decodeURIComponent(buttonText) : 'Unknown';

    await client.query(insertQuery, [broadcastId, contactId, decodedButtonText]);
    ...
    console.log(`Click logged for Contact: ${contactId} | Broadcast: ${broadcastId}`);
